...
- 주문 요청 시 쿠폰을 사용하면 쿠폰에 따라 정량/정액 할인 금액을 적용한다.
...
- 쿠폰을 사용하였는데 이미 사용한 쿠폰이면 CONFLICT 예외가 발생한다.
- 쿠폰을 사용하였는데 쿠폰이 존재하지 않으면 NOT_FOUND 예외가 발생한다.
...
#### [동시성 케이스]
- 여러 유저가 동시에 같은 상품을 주문 할 경우 재고가 정상적으로 차감되어야한다
- 한 유저가 동시에 주문을 처리할 때, 포인트가 중복적으로 차감되지 않아야한다(첫번째 주문에 대해서만 주문이 성공하고 나머지는 실패해야한다.)
- 한 유저가 동시에 주문을 처리할 때, 쿠폰이 중복적으로 사용되지 않아야한다(첫번째 주문에 대해서만 쿠폰이 사용되고 나머지는 실패해야한다.)
- 한 유저가 동시에 주문을 처리할 때, 결제가 중복적으로 이루어지지 않아야한다(첫번째 주문에 대해서만 결제가 성공하고 나머지는 실패해야한다.)
